<template>
  <div class="max-w-2xl mx-auto">
    <div class="mb-6">
      ...
      <p class="text-gray-400 mt-1 text-sm">Fill in the details below to submit a leave request</p>
    </div>

    <div class="bg-gray-900 border border-white/10 rounded-2xl p-6 shadow-xl">
      <!-- Success state -->
      <div v-if="submitted" class="text-center py-8">
        ...
        <p class="text-gray-400 text-sm mb-6">Your leave request has been sent successfully.</p>
        <button @click="resetForm" class="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-all">
          Apply Another
        </button>
      </div>
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-5">
        <!-- Error -->
        <div v-if="errorMessage" class="bg-red-500/20 border border-red-400/30 text-red-300 rounded-xl px-4 py-3 text-sm">
          {{ errorMessage }}
        </div>

        <!-- Leave Type -->
        <div>
          <label class="block text-gray-300 text-sm font-medium mb-2">Leave Type</label>
          <select v-model="form.leaveType" id="leaveType" required
            class="w-full bg-gray-800 border rounded-xl px-4 py-3 text-sm text-white transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500"
            :class="errors.leaveType ? 'border-red-500' : 'border-white/10'">
            <option value="" disabled class="bg-gray-800">Select leave type</option>
            <option v-for="type in leaveTypes" :key="type" :value="type" class="bg-gray-800">{{ type }}</option>
          </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="form.startDate" type="date" id="startDate" required
              class="w-full bg-gray-800 border rounded-xl px-4 py-3 text-sm text-white transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500"
              :class="errors.startDate ? 'border-red-500' : 'border-white/10'" />
          ...
          <div>
            ...
            <input v-model="form.endDate" type="date" id="endDate" required
              class="w-full bg-gray-800 border rounded-xl px-4 py-3 text-sm text-white transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500"
              :class="errors.endDate ? 'border-red-500' : 'border-white/10'" />
        ...
        <div>
          ...
          <textarea v-model="form.reason" id="reason" rows="4" placeholder="Briefly describe the reason for your leave..."
            required
            class="w-full bg-gray-800 border rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
            :class="errors.reason ? 'border-red-500' : 'border-white/10'">
          </textarea>
          <p v-if="errors.reason" class="text-red-400 text-xs mt-1">{{ errors.reason }}</p>
        </div>

        <button type="submit" :disabled="loading"
          class="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition-all hover:scale-[1.01] flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/40">
