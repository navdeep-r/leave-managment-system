<template>
  <div>
    <div class="mb-6">
      ...
      <p class="text-gray-400 mt-1 text-sm">Review and manage all employee leave requests</p>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-gray-900 border border-yellow-500/20 rounded-2xl p-4">
        <p class="text-yellow-400 text-xs mb-1">Pending</p>
        <p class="text-white text-2xl font-bold">{{ countByStatus('Pending') }}</p>
      </div>
      <div class="bg-gray-900 border border-green-500/20 rounded-2xl p-4">
    ...
    <!-- Empty state -->
    <div v-else-if="leaves.length === 0" class="bg-gray-900 border border-white/10 rounded-2xl p-12 text-center">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-white/10">
          <thead class="bg-gray-800/60">
            <tr>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Employee</th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Leave Type</th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Dates</th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Reason</th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</th>
              <th class="px-5 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
            ...
            <tr v-for="leave in leaves" :key="leave._id" class="hover:bg-white/3 transition-colors">
              <td class="px-5 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 bg-indigo-600/40 rounded-full flex items-center justify-center text-indigo-300 font-bold text-xs flex-shrink-0">
                    {{ initials(leave.employeeId?.name) }}
                  </div>
                  <div>
                    <p class="text-white text-sm font-medium">{{ leave.employeeId?.name }}</p>
                    <p class="text-gray-400 text-xs">{{ leave.employeeId?.email }}</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-5 py-4 text-xs text-gray-300">
                ...
                <div class="text-gray-500">to {{ formatDate(leave.endDate) }}</div>
              </td>
              <td class="px-5 py-4 text-sm text-gray-300 max-w-xs">
              ...
              <td class="px-5 py-4">
                <div v-if="leave.status === 'Pending'" class="flex gap-2">
                  <button
                    ...
                    class="bg-green-600/20 hover:bg-green-500/30 text-green-400 border border-green-500/30 text-xs font-medium px-3 py-1.5 rounded-lg transition-all disabled:opacity-50"
                  >
                    Approve
                  </button>
                  <button
                    ...
                    :disabled="actionLoading === leave._id"
                    class="bg-red-600/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 text-xs font-medium px-3 py-1.5 rounded-lg transition-all disabled:opacity-50"
                  >
                    Reject
                  </button>
                </div>
